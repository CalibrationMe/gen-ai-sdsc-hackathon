...
def prompt_string(gender = 'woman', ethnicity = 'korean', age = '50s', destination = 'Sahara', weather_lims = [35, 40], with_text_output=False):
    ...
    string_setup = f'A {gender}, {ethnicity}, aged {age} is travelling to {destination}. The weather will be between {np.min(weather_lims):.1f} to {np.max(weather_lims):.1f} degrees celsius. '
    ...
    if with_text_output: 
        string_text = f'As text output, list the outer set of clothes with basic descriptions (type of clothing, 3-4 word description for searching product online) that would be fitting for searching this clothings online. Make sure that the text output is **only** a csv compatible output and no other text.'
